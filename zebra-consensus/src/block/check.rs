...
pub fn subsidy_is_valid(block: &Block, network: &Network) -> Result<(), BlockError> {
    ...
    if height < canopy_activation_height {
        ...
        // But we require checkpoints until Canopy activation so founders reward does not apply for Zebra.
        unreachable!("we cannot verify consensus rules before Canopy activation");
    }

    // Funding streams are not paid before Canopy activation or after the third halving
